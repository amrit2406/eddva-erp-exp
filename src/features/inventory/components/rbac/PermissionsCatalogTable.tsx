...
export default function PermissionsCatalogTable({ resources }: PermissionsCatalogTableProps) {
  if (resources.length === 0) {
    return <div className="p-8 text-center text-slate-500">No permissions found</div>;
  }
          ...
          {resources.flatMap((resource) =>
            resource.permissions.map((permission, index) => (
              <tr key={permission.permission_id} className="border-b border-slate-100 hover:bg-slate-50">
                {index === 0 && (
                  <td className="py-3 px-4 align-top" rowSpan={resource.permissions.length}>
                ...
                <td className="py-3 px-4 text-slate-600 text-sm max-w-xs">{permission.description}</td>
                <td className="py-3 px-4">
                  <span
