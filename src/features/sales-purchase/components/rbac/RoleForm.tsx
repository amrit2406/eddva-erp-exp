...
import { getMyPermissions, getPermissionsCatalog } from '../../api/roles.api';
import type { Role, RoleFormData, RolePermission } from '../../types/sales-purchase.types';
...
export default function RoleForm({ role, onSubmit, submitText, isSubmitting }: RoleFormProps) {
  ...
  const catalogQuery = useQuery({ queryKey: ['sales-purchase', 'permissions-catalog'], queryFn: getPermissionsCatalog });
  const mineQuery = useQuery({
    queryKey: ['sales-purchase', 'my-permissions'],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (grantable.length === 0) {
      setGrantError("You can only give permissions that your own account has in Sales & Purchase.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      <section className={card}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Role name" error={show(errors.name)}>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Purchase Clerk" className={input} />
          </Field>
          <Field label="Who is it for?" error={show(errors.description)}>
            <input
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. Raises purchase orders and records goods received"
              className={input}
            />
          </Field>
        </div>
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <button
          type="button"
          onClick={() => navigate('/sales-purchase/roles')}
