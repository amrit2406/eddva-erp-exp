...
import type { UserAssignmentFormData } from '../../types/sales-purchase.types';
...
export default function CreateUserPage() {
  ...
  const { data: roles = [], isLoading } = useQuery({ queryKey: ['sales-purchase', 'roles'], queryFn: getRoles, enabled: isAdmin });
  ...
  const create = useMutation({
    ...
    onSuccess: (_, data) => {
      queryClient.invalidateQueries({ queryKey: ['sales-purchase', 'users'] });
      queryClient.invalidateQueries({ queryKey: ['sales-purchase', 'roles'] });
      toast.success(`${data.user_name} can now use Sales & Purchase`);
  ...
  return (
    <div className="space-y-5">
      <Link to="/sales-purchase/users" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-brand-navy">
        <ArrowLeft className="h-4 w-4" /> Users
      </Link>
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">New user</h1>
        <p className="mt-0.5 text-sm text-slate-500">Give someone a login for Sales & Purchase and choose what they can do.</p>
      </div>
      ...
      {!isAdmin ? (
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          <section className={card}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="ERP user ID" hint="Their ID in the main ERP system." error={show(errors.eddva_user_id)}>
                <input value={form.eddva_user_id} onChange={(e) => set({ eddva_user_id: e.target.value })} placeholder="e.g. usr_purchase_clerk_001" autoComplete="off" className={input} />
              </Field>
            </div>
          </section>
          ...
          <section className={card}>
            ...
            {roles.length === 0 ? (
              <p className="text-sm text-slate-600">
                ...
                <Link to="/sales-purchase/roles/new" className="font-medium text-brand hover:text-brand-navy">
                  Create a role first
                </Link>
                .
              </p>
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={() => navigate('/sales-purchase/users')}
