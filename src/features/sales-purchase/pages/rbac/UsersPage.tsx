...
export default function UsersPage() {
  ...
  const { data: users = [], isLoading, error, refetch } = useQuery({ queryKey: ['sales-purchase', 'users'], queryFn: getUserAssignments, enabled: isAdmin });

  const revoke = useMutation({
    mutationFn: (user: UserAssignment) => revokeUserAssignment(user.id),
    onSuccess: (_, user) => {
      queryClient.setQueryData<UserAssignment[]>(['sales-purchase', 'users'], (current) => current?.filter((u) => u.id !== user.id));
      toast.success(`${user.user_name} can no longer use Sales & Purchase`);
    },
  ...
  const header = (
    <ListHeader
      icon={Users}
      title="Users"
      description="People who can use Sales & Purchase, and the role that decides what each one can do."
      actions={
        isAdmin && (
          <>
            <Link to="/sales-purchase/roles" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50">
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Users} title="No users yet" message="Add the people who will raise orders, approve them or record invoices, and give each one a role." action={newButton} />
      ) : (
        <>
      ...
      <ConfirmDialog
        ...
        title="Remove access?"
        message={pendingRevoke ? `${pendingRevoke.user_name} will no longer be able to sign in to Sales & Purchase. You can add them again later.` : ''}
