...
import type { Permission, PermissionFormData } from '../../types/canteen.types';
import { ACTION_LABEL, ACTION_ORDER, actionInfo, permissionSentence } from '../../utils/permissionLabels';

const NEW = '__new__';
const OTHER = '__other__';

// "Menu Items" -> "menu_items"; "Export CSV" -> "export_csv".
...
// Existing naming style: "TOPUP Wallets".
...
export default function PermissionForm({ permission, onSubmit, submitText, isSubmitting }: PermissionFormProps) {
  ...
  const { data: existing = [], isLoading } = useQuery({ queryKey: ['canteen', 'permissions'], queryFn: getPermissions });
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      <section className="rounded-3xl bg-white p-5 sm:p-6 shadow-soft ring-1 ring-slate-200/70">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Area" error={show(errors.area)} hint={!locked ? 'Which part of Canteen this is about.' : undefined}>
