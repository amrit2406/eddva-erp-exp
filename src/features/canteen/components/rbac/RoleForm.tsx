...
import type { Role, RoleFormData, RolePermission } from '../../types/canteen.types';
...
export default function RoleForm({ role, onSubmit, submitText, isSubmitting }: RoleFormProps) {
  ...
  const catalogQuery = useQuery({ queryKey: ['canteen', 'permissions-catalog'], queryFn: getPermissionsCatalog });
  const mineQuery = useQuery({
    queryKey: ['canteen', 'my-permissions'],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (grantable.length === 0) {
      setGrantError("You can only give permissions that your own account has in Canteen.");
      return;
    }
      ...
      <section className={card}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Role name" error={show(errors.name)}>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Counter Staff" className={input} />
          </Field>
          <Field label="Who is it for?" error={show(errors.description)}>
            <input
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. Runs a counter: opens shifts, takes orders and payments"
              className={input}
            />
          </Field>
        </div>
