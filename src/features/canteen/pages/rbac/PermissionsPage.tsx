...
export default function PermissionsPage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: (_, p) => {
      queryClient.setQueryData<Permission[]>(['canteen', 'permissions'], (current) => current?.filter((x) => x.permission_id !== p.permission_id));
  ...
  return (
    <div className="space-y-5">
      <ListHeader
        icon={KeyRound}
        title="Permissions"
        description="What people are allowed to do in Canteen. Give them to people through roles."
        actions={
          <>
            <Link
              to="/canteen/roles"
      ...
      {isLoading ? (
        ...
        <EmptyState icon={KeyRound} title="No permissions yet" message="Permissions decide what each role can see and do in Canteen." />
      ) : (
        <>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by area or action, e.g. “orders” or “top up”"
