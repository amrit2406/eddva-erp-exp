...
export default function RolesPage() {
  ...
  const { data: roles = [], isLoading, error, refetch } = useQuery({ queryKey: ['canteen', 'roles'], queryFn: getRoles, enabled: isAdmin });
  // Only used to show area names instead of codes.
  const { data: catalog } = useQuery({ queryKey: ['canteen', 'permissions-catalog'], queryFn: getPermissionsCatalog, enabled: isAdmin });
  ...
  const remove = useMutation({
    ...
    onSuccess: (_, role) => {
      queryClient.setQueryData<Role[]>(['canteen', 'roles'], (current) => current?.filter((r) => r.role_id !== role.role_id));
  ...
  const header = (
    <ListHeader
      ...
      actions={
        isAdmin && (
          <>
            <Link to="/canteen/permissions" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50">
              <KeyRound className="h-4 w-4" /> Permissions
            </Link>
            <Link
              to="/canteen/roles/new"
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={Shield}
          title="No roles yet"
          message="Create a role like “Counter Staff” or “Canteen Manager”, choose what it can do, then give it to people."
          action={
            <Link to="/canteen/roles/new" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-brand-navy to-brand px-4 py-2.5 text-sm font-semibold text-white">
              <Plus className="h-4 w-4" /> New role
            </Link>
          }
        />
                      ...
                      return (
                        <tr key={role.role_id} className="transition-colors hover:bg-slate-50/80">
                          <td className="py-3.5 pl-5 pr-3">
                            <Link to={`/canteen/roles/${role.role_id}/edit`} className="font-semibold text-brand-navy hover:text-brand hover:underline">
                              {role.name}
                            </Link>
                          ...
                          <td className="py-3.5 pl-3 pr-5">
                            <div className="flex items-center justify-end gap-0.5">
                              <IconAction icon={Pencil} label="Edit role" to={`/canteen/roles/${role.role_id}/edit`} />
