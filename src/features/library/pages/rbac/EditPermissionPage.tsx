...
export default function EditPermissionPage() {
  ...
  const { data: permission, isLoading, error, refetch } = useQuery({
    queryKey: ['library', 'permission', id],
  ...
  const save = useMutation({
    ...
    onSuccess: (_, data) => {
      queryClient.invalidateQueries({ queryKey: ['library', 'permissions'] });
      queryClient.invalidateQueries({ queryKey: ['library', 'permission', id] });
      toast.success(`Permission “${data.name}” saved`);
      navigate('/library/permissions');
    },
  ...
  return (
    <div className="space-y-5">
      <Link to="/library/permissions" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-brand-navy">
