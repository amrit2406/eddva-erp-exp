...
import { getPermissions } from '../../api/library.api';
import type { Permission, PermissionFormData } from '../../types/library.types';
import { ACTION_LABEL, ACTION_ORDER, actionInfo, permissionSentence } from '../../utils/permissionLabels';

const NEW = '__new__';
const OTHER = '__other__';

// "Membership Rules" -> "membership_rules"; "Export CSV" -> "export_csv".
...
// Existing naming style: "RENEW Issue & Return".
...
export default function PermissionForm({ permission, onSubmit, submitText, isSubmitting }: PermissionFormProps) {
  ...
  const { data: existing = [], isLoading } = useQuery({ queryKey: ['library', 'permissions'], queryFn: getPermissions });
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      <section className="rounded-3xl bg-white p-5 sm:p-6 shadow-soft ring-1 ring-slate-200/70">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Area" error={show(errors.area)} hint={!locked ? 'Which part of Library this is about.' : undefined}>
          ...
          <Field label="Name" error={show(errors.name)}>
            <input
              ...
              placeholder="e.g. RENEW Issue & Return"
              className={input}
            />
          </Field>
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <button
          type="button"
          onClick={() => navigate('/library/permissions')}
